...
test_scan_num = [8, 21, 30, 31, 34, 38, 40, 41, 45, 55, 63, 82, 103, 110, 114]
...
def main():
    ...
    scan_list = list(train_dataset.keys())
    train_scan_list = [scan for scan in scan_list if int(scan.split('scan')[1]) not in test_scan_num]
    test_scan_list = [scan for scan in scan_list if int(scan.split('scan')[1]) in test_scan_num]
    ...
    for scan in scan_list:
        ...
        if scan in train_scan_list:
            ...
            refined_test_dataset_size += len(all_data)

    print(f"Train dataset size: {refined_train_dataset_size}")
    print(f"Test dataset size: {refined_test_dataset_size}")
